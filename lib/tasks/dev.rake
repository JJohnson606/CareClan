...
task sample_data: :environment do
  if Rails.env.production?
    puts 'Are you sure you want to run this in production? (yes/no)'
    input = STDIN.gets.chomp
    exit unless input.downcase == 'yes'
  end
  ...
  ActiveRecord::Base.transaction do
    ...
    doctors = [
      ...
      { email: 'orthopedist@example.com', password: 'password', name: 'Dr. Jane Setter', role: 1,
        image_file: 'Jane.jpeg' },
      { email: 'gp@example.com', password: 'password', name: 'Dr. Samuel Green', role: 1, image_file: 'Samuel.jpeg' },
      { email: 'therapist@example.com', password: 'password', name: 'Dr. Alma Chavez', role: 1,
        image_file: 'Alma.jpeg' }
    ].map do |doc_data|
      doc_image_path = Rails.root.join('app', 'assets', 'images', doc_data[:image_file])
      doctor = User.create(email: doc_data[:email], password: doc_data[:password], name: doc_data[:name],
                           role: doc_data[:role], trust: doc_data[:trust])
      if File.exist?(doc_image_path)
        doctor.profile_picture.attach(io: File.open(doc_image_path), filename: File.basename(doc_image_path),
    ...
    family_and_friends = [
      ...
      { email: 'emily@example.com', password: 'password', name: 'Emily Johnson', role: 3, trust: false,
        image_file: 'Emily.jpeg' },
      { email: 'frank@example.com', password: 'password', name: 'Frank Brown', role: 3, trust: true,
        image_file: 'Frank.jpeg' },
      { email: 'grace@example.com', password: 'password', name: 'Grace Davis', role: 2, trust: false,
        image_file: 'Grace.jpeg' },
      { email: 'harry@example.com', password: 'password', name: 'Harry Wilson', role: 3, trust: true,
        image_file: 'Harry.jpeg' },
      { email: 'irene@example.com', password: 'password', name: 'Irene Morgan', role: 3, trust: false,
        image_file: 'Irene.jpeg' },
      { email: 'jack@example.com', password: 'password', name: 'Jack Lee', role: 3, trust: true,
        image_file: 'Jack.jpeg' },
      { email: 'kelly@example.com', password: 'password', name: 'Kelly Young', role: 3, trust: false,
        image_file: 'Kelly.jpeg' },
      ...
      person = User.create(email: person_data[:email], password: person_data[:password], name: person_data[:name],
                           role: person_data[:role], trust: person_data[:trust], relationship_to_patient: relationships.sample)
      if File.exist?(person_image_path)
        person.profile_picture.attach(io: File.open(person_image_path), filename: File.basename(person_image_path),
                                      content_type: 'image/jpeg')
      end
    ...
    # Clan creation
    example_clan = Clan.find_or_create_by!(name: "Gary's Recovery Warriors",
                                           description: 'An example clan for users sharing recovery stories and support for each other.')
    puts "Clan 'Nixon Clan' either found or created."

    # Adding users to the clan
    family_and_friends.each do |user|
      ClanMembership.find_or_create_by!(clan: example_clan, user:, role: 'member')
    end
    ...
    MedicalRecord.create!(
      record_type: 'imaging',
      record_date: Date.today - 30.days,
    ...
    MedicalRecord.create!(
      ...
      record_date: Date.today - 30.days,
      notes: { condition: 'Fractured Hand', symptoms: 'Pain and swelling',
               recommended_treatment: 'Orthopedic consultation and casting' }.to_json,
      patient_id: gary.id,
    ...
    MedicalRecord.create!(
      ...
      record_date: Date.today - 23.days,
      notes: { treatment: 'Casting and pain management', duration: '6 weeks',
               follow_up: 'Regular check-ups and potential physical therapy' }.to_json,
      patient_id: gary.id,
      created_by_id: doctors[1].id
    )
    ...
    MedicalRecord.create!(
      ...
      record_date: Date.today - 23.days,
      notes: { medication_name: 'Ibuprofen', dosage: '400 mg every 8 hours', duration: '2 weeks',
               purpose: 'Manage pain and inflammation' }.to_json,
      patient_id: gary.id,
    ...
    User.where(role: :patient).each do |patient|
      10.times do
        ...
        when 'treatment_plan'
          treatments = ['Physical therapy sessions', 'Adjustment of medication regimen', 'Dietary changes',
                        'Scheduled surgery']
          notes = {
        ...
        when 'prescription'
          medications = %w[Amoxicillin Ibuprofen Paracetamol]
          notes = {
        ...
        when 'lab_results'
          ...
          test_results = {
            'CBC' => {
              'WBC Count' => { normal: (4000..11_000), optimal: (5000..10_000), clinical: (2000..17_000) },
              ...
              'Platelet Count' => { normal: (150_000..450_000), optimal: (180_000..400_000),
                                    clinical: (100_000..500_000) }
            },
            'Lipid Panel' => {
          ...
          test_results[selected_test].each do |test, ranges|
            ...
            (optimal_range_start..optimal_range_end)
        ...
        when 'surgical_reports'
          ...
          outcomes = ['Procedure successful, patient recovering well', 'Complications encountered, monitoring closely',
                      'Unremarkable post-operative course', 'Patient discharged with follow-up care instructions']
          notes = {
    ...
    Comment.find_each do |comment|
      family_and_friends.each do |family_member|
        ...
        if comment.vote_registered?
          puts "#{family_member.name} #{vote_type ? 'approved' : 'disapproved'} comment #{comment.id}."
        end
      end
    end
